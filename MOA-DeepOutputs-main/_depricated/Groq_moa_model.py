...
class MixtureOfAgents:
    def __init__(self, models: List[str], num_layers: int = 2):
        self.agents = [GroqAgent(f"Agent {i+1}", model, f"Agent {i+1}") for i, model in enumerate(models)]
        self.num_layers = num_layers
        self.synthesis_agent = GroqAgent("Synthesis Agent", "llama3-70b-8192", "Synthesis Agent")
        self.final_agent = GroqAgent("Final Agent", "llama3-70b-8192", "Final Agent")
...
async def main():
    ...
    # Create MixtureOfAgents with Groq models
    moa = MixtureOfAgents(models, num_layers=2)  # Changed to default 2 layers
